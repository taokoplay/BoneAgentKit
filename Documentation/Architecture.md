...
BoneAgentKit 以独立 Swift Package 交付，依赖方向为 `App Host → BoneAgentKit`，Kit 不反向引用 Host 模型、数据库或 UI。
...
Host 的 Catalog、模型发现、Provider 连通性、图片和文本执行均可使用这条链路。设置 UI、持久化与用户配置保留在 App Host，只映射到 BoneInference 公开类型；BoneAgentKit 不依赖 Host 的业务模块。
